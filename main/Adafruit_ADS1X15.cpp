...
*/
/**************************************************************************/
void Adafruit_ADS1X15::begin(int i2c_num, uint8_t i2c_addr ) {
	  m_i2c_dev = new I2CDevice(i2c_num, i2c_addr);
}
...
void Adafruit_ADS1X15::writeRegister(uint8_t reg, uint16_t value) {
  buffer[0] = value >> 8;
  buffer[1] = value & 0xFF;
  m_i2c_dev->write_bulk(reg, buffer, 2);
}
...
uint16_t Adafruit_ADS1X15::readRegister(uint8_t reg) {
  m_i2c_dev->read_bulk(reg, buffer, 2);
